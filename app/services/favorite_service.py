...
class FavoriteService:
    ...
    async def create_favorite(
        self, db: Session, favorite_data: FavoritePokemonCreate
    ) -> FavoritePokemon:
        ...
        existing = (
            db.query(FavoritePokemon)
            .filter(FavoritePokemon.pokemon_id == favorite_data.pokemon_id)
            .first()
        )
    ...
    def get_favorites(
        self, db: Session, skip: int = 0, limit: int = 100, only_favorites: bool = True
        ...
        total = query.count()
        favorites = (
            query.order_by(desc(FavoritePokemon.created_at))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_favorite_by_id(self, db: Session, favorite_id: int) -> FavoritePokemon:
        ...
        favorite = (
            db.query(FavoritePokemon).filter(FavoritePokemon.id == favorite_id).first()
        )
        if not favorite:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
    ...
    def get_favorite_by_pokemon_id(
        self, db: Session, pokemon_id: int
    ) -> Optional[FavoritePokemon]:
        """Get favorite pokemon by pokemon_id"""
        return (
            db.query(FavoritePokemon)
            .filter(FavoritePokemon.pokemon_id == pokemon_id)
            .first()
        )

    def update_favorite(
        self, db: Session, favorite_id: int, update_data: FavoritePokemonUpdate
    ) -> FavoritePokemon:
